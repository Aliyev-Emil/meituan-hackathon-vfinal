...
| Route | Purpose |
|-------|---------|
| `/` | LLM chat planner, multiple plan cards, one-stop agent |
...
- I'm gonna be free the whole day, make some arrangements for me and my wife who is on a diet.
- Create an afternoon plan with cantonese cuisine + quiet place for parents.
- I'm gonna be with 3 friends of mine, We want Japanese/lighter food in Futian.
- Quick lunch nearby.
...
See [Design.pdf](./Design.pdf) (≤2 pages).

## Test users

- `xiaoming` (current user)
- `zhangwei`, `lina`, `wangfang`, and etc. — add as friends by ID
...
Cultra treats user input as a **goal**, not a keyword search. The system parses intent, searches mock Shenzhen venues, ranks combinations, and returns multiple alternative, swipeable plans consisting of a full multi-stop itinerary.
